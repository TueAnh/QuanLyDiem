...
namespace QuanLyDiem.DAL
{
	class HocVienDT_DAL
	{
        QuanLyDiemEntitiess db;
		public string getHVDAL(string str)
		{
            db = new QuanLyDiemEntitiess();
			...
			foreach (var x in v)
			{
				if (mhk != x.HocPhan.MaHK)
				{
					if (nh != "")
					{
						...
						dtbT4 = Math.Round(dtbT10 * 4 / 10, 2);
					...
					dtbm = Convert.ToDouble(x.DiemBT) * Convert.ToDouble(x.HocPhan.PhanTramDKT)
						+ Convert.ToDouble(x.DiemGK) * Convert.ToDouble(x.HocPhan.PhanTramDKT)
